...
// Los estilos de 'confirm' usan las variables CSS del tenant
// que se establecen dinámicamente en TenantContext
const getStyles = (type: ModalType) => {
  const styles = {
    ...
    confirm: {
      // Usa CSS custom properties para colores dinámicos del tenant
      iconBgStyle: { background: 'linear-gradient(to bottom right, var(--tenant-primario), var(--tenant-dark))' },
      borderStyle: { borderColor: 'var(--tenant-primario)', opacity: 0.2 },
      bgColor: 'bg-blue-50',
      textColorStyle: { color: 'var(--tenant-primario)' },
      buttonBgStyle: { background: 'linear-gradient(to right, var(--tenant-primario), var(--tenant-dark))' },
      // Fallback classes for non-dynamic styles
      iconBg: '',
      borderColor: 'border-blue-200',
      textColor: '',
      buttonBg: '',
...
const Modal: React.FC<ModalProps> = ({
  ...
  // Para tipo 'confirm', usamos estilos inline dinámicos del tenant
  const isConfirm = type === 'confirm';
  const iconBgStyle = isConfirm && 'iconBgStyle' in styles ? styles.iconBgStyle : undefined;
  const textColorStyle = isConfirm && 'textColorStyle' in styles ? styles.textColorStyle : undefined;
  const buttonBgStyle = isConfirm && 'buttonBgStyle' in styles ? styles.buttonBgStyle : undefined;
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in-up">
      <div className="animate-slide-down bg-white rounded-3xl p-8 shadow-2xl max-w-md mx-4">
        <div className="flex flex-col items-center text-center">
          {/* Icono animado */}
          <div 
            className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 ${type !== 'loading' ? 'animate-pulse' : ''} ${!isConfirm ? `bg-gradient-to-br ${styles.iconBg}` : ''}`}
            style={iconBgStyle}
          >
            {defaultIcon}
          </div>
            ...
            {buttons && buttons.length > 0 ? (
              buttons.map((button, index) => (
                <button
                  ...
                  style={button.variant === 'primary' || !button.variant ? (isConfirm ? buttonBgStyle : undefined) : undefined}
                >
                  {button.label}
                </button>
              ))
              ...
              <button
                onClick={onClose}
                disabled={type === 'loading'}
                className={`w-full px-6 py-3 text-white font-semibold rounded-lg transition-all duration-300 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${isConfirm ? styles.buttonHover : `bg-gradient-to-r ${styles.buttonBg} ${styles.buttonHover}`}`}
                style={isConfirm ? buttonBgStyle : undefined}
              >
                Cerrar
              </button>
            )}

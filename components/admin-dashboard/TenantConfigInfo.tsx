  ...
  return (
    <div className="bg-surface rounded-2xl shadow-sm border border-edge p-4 sm:p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        <div className="p-3 bg-canvas rounded-xl min-w-0">
          <p className="text-xs text-muted">Slug</p>
          <p className="text-sm font-mono text-label mt-1 truncate">{tenant.slug}</p>
        </div>
        <div className="p-3 bg-canvas rounded-xl min-w-0">
          <p className="text-xs text-muted">Color primario</p>
          <div className="flex items-center gap-2 mt-1 min-w-0">
            <div className="w-5 h-5 rounded-md flex-shrink-0" style={{ backgroundColor: tenant.color_primario }} />
            <span className="text-sm font-mono text-label truncate">{tenant.color_primario}</span>
          </div>
        </div>
